...
-- ============================================================
-- PROFESSIONAL SINGLE-LINE NOTIFICATION SYSTEM
-- Reuses one card, suppresses duplicates, and avoids stacking GUIs.
-- ============================================================
local function NotificacaoNightMystic(titulo, mensagem)
    if not mensagem or tostring(mensagem) == "" then return end

    task.spawn(function()
        local TweenService = Services.TweenService
        local CoreGui = Services.CoreGui
        local guiName = "VelaHubNotify"
        local now = os.clock()
        local state = _G.__VelaNotifyState or {
            LastMessage = "",
            LastTime = 0,
            Version = 0,
        }
        _G.__VelaNotifyState = state

        if state.LastShownTime and (now - state.LastShownTime) < 0.6 then
            return
        end
        state.LastShownTime = now

        local messageText = tostring(mensagem)
        if state.LastMessage == messageText and (now - state.LastTime) < 2 then
            return
        end
        ...
        local frame, titleLabel, msgLabel, stroke, icon

        if gui then
            frame = gui:FindFirstChild("Card")
            titleLabel = frame and frame:FindFirstChild("Title")
            msgLabel = frame and frame:FindFirstChild("Message")
            stroke = frame and frame:FindFirstChildOfClass("UIStroke")
            icon = frame and frame:FindFirstChild("Icon")
        end

        if not gui or not frame or not titleLabel or not msgLabel then
            ...
            frame.Position = UDim2.new(1, 24, 0.84, 0)
            frame.Size = UDim2.new(0, 310, 0, 68)
            ...
            icon.Image = "rbxassetid://73902065845651"
            ...
            msgLabel.Position = UDim2.new(0, 70, 0, 30)
            msgLabel.Size = UDim2.new(1, -82, 0, 26)
            msgLabel.Font = Enum.Font.Gotham
            msgLabel.TextColor3 = Color3.fromRGB(175, 175, 185)
        ...
        titleLabel.Text = tostring(titulo or "Vela Hub")
        ...
        TweenService:Create(
            ...
            {Position = UDim2.new(1, -334, 0.84, 0)}
        ):Play()

        task.wait(2.4)
        if state.Version ~= version or not gui.Parent then return end

        local out = TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
        TweenService:Create(frame, out, {Position = UDim2.new(1, 24, 0.84, 0), BackgroundTransparency = 1}):Play()
        ...
        TweenService:Create(msgLabel, out, {TextTransparency = 1}):Play()
        task.wait(0.3)
        if state.Version == version and gui.Parent then
...
local FolderName = "Vela Hub  [Beta]"
...
do
    ...
    -- Wait for the local character/data to be ready before initializing
    -- modules that require HumanoidRootPart, Level, or Energy.
    ...
    local dataFolder = plr:WaitForChild("Data", 30)
    local levelValue = dataFolder and dataFolder:WaitForChild("Level", 30)
    local energyValue = character:WaitForChild("Energy", 30)

    -- If the character was still loading, refresh references once.
    if not energyValue then
        character = plr.Character or plr.CharacterAdded:Wait()
    ...
    Lv = levelValue and levelValue.Value or 1
...
elseif placeId == 7449423635 or placeId == 100117331123089 then
    ...
    plr:Kick("❌ Error - Script only supports Blox Fruits.")
end
...
_G.BringSpeed = _G.BringSpeed or 1800
...
--==================================================
-- SMOOTH BRING SYSTEM
-- Reuses mob targets and avoids re-tweening mobs that are already settled.
--==================================================
_G.BringRetargetThreshold = _G.BringRetargetThreshold or 6
_G.BringTweenDuration = _G.BringTweenDuration or 0.65
...
BringEnemy = function()
    ...
    pcall(function()
        sethiddenproperty(playerObj, "SimulationRadius", math.huge)
    end)
    ...
    for _, mob in ipairs(enemies) do
        ...
        if hum and root and hum.Health > 0 and not IsRaidMob(mob) then
            ...
            if dist <= (_G.BringRange or 235) then
                local isTweening = root:GetAttribute("Tweening") == true
                local previousTarget = root:GetAttribute("BringTarget")
                local lastUpdate = root:GetAttribute("BringUpdate") or 0
                local targetChanged = not previousTarget or (previousTarget - targetPos).Magnitude > (_G.BringRetargetThreshold or 6)

                if (not isTweening) and (targetChanged or (now - lastUpdate) > 1.25) then
                    count += 1
                    root:SetAttribute("Tweening", true)
                    root:SetAttribute("BringTarget", targetPos)
                    root:SetAttribute("BringUpdate", now)

                    local distance = (root.Position - targetPos).Magnitude
                    local duration = math.clamp(
                        distance / math.max((_G.BringSpeed or 1800), 1),
                        0.20,
                        (_G.BringTweenDuration or 0.65)
                    )
                    ...
                    tween.Completed:Once(function()
                        if root and root.Parent then
                            ...
                            root:SetAttribute("BringUpdate", os.clock())
                        end
                    end)
                    tween:Play()
                end
...
--==================================================
-- SMOOTH BRING CONTROLLER
-- Small polling interval; BringEnemy itself prevents duplicate retweens.
--==================================================
task.spawn(function()
    while task.wait(0.25) do
        if FarmAtivo() and _B then
            BringEnemy()
        end
    end
end)
...
-- =======================
-- SMOOTH DISTANCE TWEEN
-- =======================
-- Keep one active tween, avoid needless restarts, and use smoother easing.
getgenv().TweenSpeedFar = getgenv().TweenSpeedFar or 250
getgenv().TweenSpeedNear = getgenv().TweenSpeedNear or 300

local ActiveTween = nil
local ActiveTweenTarget = nil
local ActiveTweenToken = 0
local LastTweenStart = 0
local TweenRetargetThreshold = 8
local TweenMinRestartDelay = 0.08
...
local function FinishTweenState(token)
    if token ~= ActiveTweenToken then return end
    ActiveTween = nil
    ActiveTweenTarget = nil
    getgenv().OnFarm = true
end
...
_tp = function(targetCFrame)
    ...
    if not humanoid or not hrp or not targetCFrame then return end
    ...
    -- Do not rebuild the tween for small target changes while it is moving.
    if ActiveTween and ActiveTween.PlaybackState == Enum.PlaybackState.Playing and ActiveTweenTarget then
        local targetDelta = (targetCFrame.Position - ActiveTweenTarget.Position).Magnitude
        if targetDelta <= TweenRetargetThreshold then
            return
        end
    end

    local now = os.clock()
    if ActiveTween and now - LastTweenStart < TweenMinRestartDelay then
        return
    end

    ActiveTweenToken += 1
    local token = ActiveTweenToken
    if ActiveTween then
        pcall(function() ActiveTween:Cancel() end)
    end

    ActiveTweenTarget = targetCFrame
    LastTweenStart = now
    getgenv().OnFarm = true
    ...
    local duration = math.clamp(distance / math.max(speed, 1), 0.08, 1.75)
    ...
    ActiveTween = TweenService:Create(C, info, { CFrame = targetCFrame })
    ActiveTween.Completed:Once(function()
        FinishTweenState(token)
    end)
    ActiveTween:Play()
end
...
-- ========================================
_G.VI = {
    ["Auto Farm main"] = "Auto Farm main",
    ["Select Weapon"] = "Select Weapon",
    ["Select Farm Mode"] = "Select Farm Mode",
    ["Start Farm"] = "Start Farm",
    ["Accept Quests"] = "Accept Quests",
    ["Other"] = "Other",
    ["Kill Mobs Nearest"] = "Kill Mobs Nearest",
    ["Auto Pirate Raid"] = "Auto Pirate Raid",
    ["Collect"] = "Collect",
    ["Auto Collect Chest"] = "Auto Collect Chest",
    ["Auto Collect Berry"] = "Auto Collect Berry",
    ["Material"] = "Material",
    ["Select Material"] = "Select Material",
    ["Auto Collect Material"] = "Auto Collect Material",
    ["Auto Farm"] = "Auto Farm",
    ["Anti AFK"] = "Anti AFK",
    ["Auto Active Haki"] = "Auto Active Haki",
    ["Auto Active Cores"] = "Auto Active Cores",
    ["Auto Active V3"] = "Auto Active V3",
    ["Auto Active V4"] = "Auto Active V4",
    ["Kill Mobs"] = "Kill Mobs",
    ["Accept Quest Boss"] = "Accept Quest Boss",
    ["Accept Quest Bypass [Risk]"] = "Accept Quest Bypass [Risk]",
    ["Accept Allies"] = "Accept Allies",
    ["Tab Farm Maestry"] = "Tab Farm Mastery",
    ["Mastery Farm"] = "Mastery Farm",
    ["Auto Mastery"] = "Auto Mastery",
    ["Select Mastery"] = "Select Mastery",
    ["Fruit Skills"] = "Fruit Skills",
    ["Use Z Skill"] = "Use Z Skill",
    ["Use X Skill"] = "Use X Skill",
    ["Use C Skill"] = "Use C Skill",
    ["Use V Skill"] = "Use V Skill",
    ["Use F Skill"] = "Use F Skill",
    ["Teleport"] = "Teleport",
    ["Select Island"] = "Select Island",
    ["Teleport To"] = "Teleport To",
    ["Teleport Player"] = "Teleport Player",
    ["Stats"] = "Stats",
    ["Add Points Melee"] = "Add Points Melee",
    ["Add Points Defense"] = "Add Points Defense",
    ["Add Points Sword"] = "Add Points Sword",
    ["Add Points Gun"] = "Add Points Gun",
    ["Add Points Fruit"] = "Add Points Fruit",
    ["ESP"] = "ESP",
    ...
    ["Sea Event"] = "Sea Event",
    ["Auto Attack Sea Beast"] = "Auto Attack Sea Beast",
    ["Auto Attack Fish Boat"] = "Auto Attack Fish Boat",
    ["Auto Attack Leviathan"] = "Auto Attack Leviathan",
    ["Auto Attack Pirate GrandBrigade"] = "Auto Attack Pirate GrandBrigade",
    ["Race Upgrade"] = "Race Upgrade",
    ["Auto Race"] = "Auto Race",
    ["Select Race"] = "Select Race",
    ["Raiding"] = "Raid",
    ["Select Chip"] = "Select Chip",
    ["Buy Chip With Fruit"] = "Buy Chip With Fruit",
    ["Raid Farming"] = "Raid Farming",
    ["Auto Start Raid"] = "Auto Start Raid",
    ["Auto Complete Raid"] = "Auto Complete Raid",
    ["Auto Awakening"] = "Auto Awakening",
    ["Fruits Options"] = "Fruits Options",
    ["Select Fruit Stock"] = "Select Fruit Stock",
    ["Buy Basic Stock"] = "Buy Basic Stock",
    ["Settings"] = "Settings",
    ["FPS Boost"] = "FPS Boost",
    ["Anti Lag"] = "Anti Lag",
    ["Render Quality"] = "Render Quality",
    ["Low CPU"] = "Low CPU",
    ["Auto RAM Clean"] = "Auto RAM Clean",
    ["Remove Effects"] = "Remove Effects",
    ["Tween Speed"] = "Tween Speed",
    ["Bring Range"] = "Bring Range",
    ["Get Items"] = "Get Items",
    ["Auto Get"] = "Auto Get",
    ...
    ["Auto Ken"] = "Auto Ken",
    ["Aimbot"] = "Aimbot",
    ["Aimbot Cam Lock"] = "Aimbot Cam Lock",
    ["Aimbot Skills"] = "Aimbot Skills",
    ["Agility"] = "Agility",
    ["Auto Angel V2/V3"] = "Auto Angel V2/V3",
    ["Auto Attack"] = "Auto Attack",
    ["Auto Berry"] = "Auto Berry",
    ["Auto Boss"] = "Auto Boss",
    ["Auto Ectoplasm"] = "Auto Ectoplasm",
    ["Auto Factory"] = "Auto Factory",
    ["Auto Find Mirage"] = "Auto Find Mirage",
    ["Auto Holy Torch"] = "Auto Holy Torch",
    ["Auto Kilo"] = "Auto Kilo",
    ["Auto Pole V2"] = "Auto Pole V2",
    ["Auto Saber"] = "Auto Saber",
    ["Auto Tushita"] = "Auto Tushita",
    ["Auto Yama"] = "Auto Yama",
    ["Find Mirage Island"] = "Find Mirage Island",
    ...
    ["Select Boss"] = "Select Boss",
    ["Select Enemy"] = "Select Enemy",
    ["Select Player"] = "Select Player",
    ["Time Zone"] = "Time Zone",
    ["Full Moon"] = "Full Moon",
    ["Bring Mobs"] = "Bring Mobs",
    ["Max Bring Mobs"] = "Max Bring Mobs",
    ["Mob Height"] = "Mob Height",
    ["Use Sword Skills"] = "Use Sword Skills",
    ["Use Melee Skills"] = "Use Melee Skills",
    ["Use Gun Skills"] = "Use Gun Skills",
    ["Kill Range"] = "Kill Range",
    ["Auto Kill"] = "Auto Kill",
    ...
    ["Credits"] = "Credits",
    ["Version"] = "Version",
    ["Dimension Killed"] = "Dimension Killed",
    ["Rip_Indra"] = "Rip_Indra",
    ...
    ["Bone"] = "Bone",
    ["Time"] = "Time",
    ["Automatically start the Raid and wait 10s between checks"] = "Automatically start the Raid and wait 10s between checks",
    ["Use your lowest fruit in your bag"] = "Use your lowest fruit in your bag",
    ["Abilities Shop"] = "Abilities Shop",
    ["Adv"] = "Adv",
    ["Auto Active"] = "Auto Active",    [" Number Cursed dual katana quests "] = "Số Quest Kiếm Nguyền Rủa",
    [" Tiers V4 Status "] = " Tiers V4 Status ",
    ["Activate Boat Speed"] = "Activate Boat Speed",
    ["Delete the save file and restore defaults"] = "Delete the save file and restore defaults",
    ["Auto Bisento V2"] = "Auto Bisento V2",
    ["Auto Black Spikey"] = "Auto Black Spikey",
    ["Auto Buy Bait"] = "Auto Buy Bait",
    ["Auto Buy Boat & Move to Zone"] = "Auto Buy Boat & Move to Zone",
    ["Auto Buy Legendary Sword"] = "Auto Buy Legendary Sword",
    ["Auto Canvendish Sword"] = "Auto Canvendish Sword",
    ["Auto Citizen Quest"] = "Auto Citizen Quest",
    ["Auto Collect Azure Ember"] = "Auto Collect Azure Ember",
    ["Auto Collect Dino Bones"] = "Auto Collect Dino Bones",
    ["Auto Collect Dragon Eggs"] = "Auto Collect Dragon Eggs",
    ["Auto Collect Fruit"] = "Auto Collect Fruit",
    ["Auto Collect Gear"] = "Auto Collect Gear",
    ["Auto Collect Mirage Chest"] = "Auto Collect Mirage Chest",
    ["Auto Complete Quest"] = "Auto Complete Quest",
    ["Auto Complete Trial Race"] = "Auto Complete Trial Race",
    ["Auto Craft Volcanic Magnet"] = "Auto Craft Volcanic Magnet",
    ["Auto Darkbeard"] = "Auto Darkbeard",
    ["Auto Dojo Trainer"] = "Auto Dojo Trainer",
    ["Auto Dragon Hunter"] = "Auto Dragon Hunter",
    ["Auto Dragon Trident"] = "Auto Dragon Trident",
    ["Auto Drop Fruit"] = "Auto Drop Fruit",
    ["Auto Elite Quest"] = "Auto Elite Quest",
    ["Auto Event Prehistoric Island"] = "Auto Event Prehistoric Island",
    ["Auto Factory Raid"] = "Auto Factory Raid",
    ["Auto Farm 600 In Swords"] = "Auto Farm 600 In Swords",
    ["Auto Farm Boss Select"] = "Auto Farm Boss Select",
    ["Auto Farm Mastery Fruit"] = "Auto Farm Mastery Fruit",
    ["Auto Farm Mastery Gun"] = "Auto Farm Mastery Gun",
    ["Auto Farm Material Skull Guitar"] = "Auto Farm Material Skull Guitar",
    ["Auto Farm Observation"] = "Auto Farm Observation",
    ["Auto Find Kitsune Island"] = "Auto Find Kitsune Island",
    ["Auto Find Mirage Island"] = "Auto Find Mirage Island",
    ["Auto Find Prehistoric Island"] = "Auto Find Prehistoric Island",
    ["Auto Fish Crew Member"] = "Auto Fish Crew Member",
    ["Auto Fishing"] = "Auto Fishing",
    ["Auto Get CDK [ Last Quest ]"] = "Auto Get CDK [ Last Quest ]",
    ["Auto Haunted Crew Member"] = "Auto Haunted Crew Member",
    ["Auto Human V2/V3"] = "Auto Human V2/V3",
    ["Auto Kill Player After Trial"] = "Auto Kill Player After Trial",
    ["Auto Kill Rip Indra"] = "Auto Kill Rip Indra",
    ["Auto Law Raid"] = "Auto Law Raid",
    ["Auto Lei Accessory"] = "Auto Lei Accessory",
    ["Auto Long Sword"] = "Auto Long Sword",
    ["Auto Look At Moon"] = "Auto Look At Moon",
    ["Auto Marine Coat"] = "Auto Marine Coat",
    ["Auto Midnight Blade"] = "Auto Midnight Blade",
    ["Auto Mink V3/V4"] = "Auto Mink V3/V4",
    ["Auto Phoenix Flame"] = "Auto Phoenix Flame",
    ["Auto Pull Lever"] = "Auto Pull Lever",
    ["Auto Quest Fishing"] = "Auto Quest Fishing",
    ["Auto Quest Mirage"] = "Auto Quest Mirage",
    ["Auto Race Evolution"] = "Auto Race Evolution",
    ["Auto Sell Fish"] = "Auto Sell Fish",
    ["Auto Serpent Bow"] = "Auto Serpent Bow",
    ["Auto Skypiea V3"] = "Auto Skypiea V3",
    ["Auto Soul Guitar"] = "Auto Soul Guitar",
    ["Auto Swan"] = "Auto Swan",
    ["Auto Teleport Race Doors"] = "Auto Teleport Race Doors",
    ["Auto Train V4"] = "Auto Train V4",
    ["Auto Travel Dressrosa"] = "Auto Travel Dressrosa",
    ["Auto Tushita V2"] = "Auto Tushita V2",
    ["Auto Unawakened Haki"] = "Auto Unawakened Haki",
    ["Auto Upgrade Dragon"] = "Auto Upgrade Dragon",
    ["Auto use skill of the rod"] = "Auto use skill of the rod",
    ["Auto Warden"] = "Auto Warden",
    ["Boss Farm"] = "Boss Farm",
    ["Buso/Aura Colours"] = "Buso/Aura Colours",
    ["Automatically craft the selected bait"] = "Automatically craft the selected bait",
    ["Cursed Swords"] = "Cursed Swords",
    ["Dark Dragger + Valkyrie"] = "Dark Dragger + Valkyrie",
    ["Automatically turn in the quest"] = "Automatically turn in the quest",
    ["Fishing"] = "Fishing",
    ["Get Bounty"] = "Get Bounty",
    ["Instant Kill"] = "Instant Kill",
    ["Kills sea beasts automatically"] = "Kills sea beasts automatically",
    ["Automatically cast and reel the fishing rod"] = "Automatically cast and reel the fishing rod",
    ["Lv Bounty Farm"] = "Lv Bounty Farm",
    ["Manual Save"] = "Manual Save",
    ["Get the quest from the Angler NPC"] = "Get the quest from the Angler NPC",
    ["Quest Boss Farm"] = "Quest Boss Farm",
    ["Quests"] = "Quests",
    ["Select Bait"] = "Select Bait",
    ["Select Fishing Rod"] = "Select Fishing Rod",
    ["Setting Farm"] = "Setting Farm",
    ["Spam the Z skill of the equipped tool"] = "Spam the Z skill of the equipped tool",
    ["Teleport to Ancient Clock"] = "Teleport to Ancient Clock",
    ["Teleport to Ancient One"] = "Teleport to Ancient One",
    ["Teleport to Temple of Time"] = "Teleport to Temple of Time",
    ["Tiers V4 Status"] = "Tiers V4 Status",
    ["Trials Quests / Misc V4"] = "Trials Quests / Misc V4",
    ["turn on for farm tier + auto upgrade your tier level"] = "turn on for farm tier + auto upgrade your tier level",
    ["turn on for kill player after the race trials"] = "turn on for kill player after the race trials",
    ["Automatically sell fish"] = "Automatically sell fish",
    ["Select"] = "Select",
    ["Bones"] = "Bones",
    ["Auto Rainbow Haki"] = "Auto Rainbow Haki",
    ["Auto Teleport to Race Doors"] = "Auto Teleport to Race Doors",
...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/VTDROBLOX/Animehub/refs/heads/main/ui_tay.txt"))():MakeWindow({
    Title = "Vela Hub  [Beta]",
    SubTitle = "Script Beta!",
    SaveFolder = "VelaHub.json"
})
-- Criar ScreenGui
local screenGui = Instance.new("ScreenGui")
screenGui.Name = "VelaToggleGui"
...
uiStroke.Color = Color3.fromRGB(220, 45, 90)

local UIS = game:GetService("UserInputService")
imageButton.MouseEnter:Connect(function() imageButton.ImageTransparency = 0.12 end)
imageButton.MouseLeave:Connect(function() imageButton.ImageTransparency = 0 end)
...
LangTab:AddSection({ Name = "Language Settings" })
LangTab:AddParagraph({
    Title = "ℹ️ Language",
    Content = "English only — all UI text has been standardized to English."
})
Discord:AddDiscordInvite({
    Name = "Vela Hub Community",
    Description = "Join My Discord :P",
...
Setting:AddButton({
    ...
    Description = "Create a manual backup of your current settings",
    Callback = function()
...
Setting:AddButton({
    Name = "Reset Settings",
    Description = "Delete the save file and restore defaults",
    Callback = function()
        ...
        if isfile and isfile(FullPath) then
            ...
            game.StarterGui:SetCore("SendNotification", {
                Title = "Vela Hub",
                Text = "No save file found to delete.",
                Duration = 3
            })
        end
    end
...
Setting:AddButton({
    ...
    Callback = function()
        ...
        -- Stop the current smooth tween cleanly.
        StopActiveTween()
        shouldTween = false
        ...
        -- Stop following the helper part until the next farm movement starts.
...
-- Fishing notifications use the same single-card Vela Hub notification UI.
local function NotifyNightMystic(texto)
    NotificacaoNightMystic("Auto Fish", texto)
end
...
Others:AddToggle({
    Name = T("Auto Fishing"),
    Description = T("Automatically cast and reel the fishing rod"),
    -- Carrega se estava ligado
...
Others:AddToggle({
    ...
    Description = T("Get the quest from the Angler NPC"),
    -- Carrega se estava ligado
...
Event:AddToggle({
    ...
    Description = "Fly to the highest point of Mirage (ideal for viewing the Moon)",
    -- 1. Carrega o estado salvo
...
Event:AddToggle({
    ...
    Description = "Fly to the advanced fruit dealer on Mirage",
    -- 1. Carrega se o teleporte estava ativo
...
Dojo:AddToggle({
	Name = T("Fly"),
	Description = "Analog-controlled flight (automatic ascent)",
	Default = false,
	Callback = function(v)
		_G.Fly = v
	end,
...
Fruit:AddToggle({
    ...
    Description = T("Automatically start the Raid and wait 10s between checks"),
    Default = false,
    Callback = function(I)
        _G.Auto_StartRaid = I;
    end,
...
Setting:AddButton({
    ...
    Description = "Run everything: remove effects, clean RAM, lower render quality, disable shadows",
    Callback = function()
        LowCpu()
        NotificacaoNightMystic("Vela Hub", "✅ Ultra Optimizer enabled!")
    end
});

Setting:AddButton({
    Name = "🧹 Clean RAM Now",
    Description = "Remove junk, dead enemies, and run garbage collection",
    Callback = function()
        pcall(function()
            -- Xóa dead enemies
            for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
        ...
        pcall(function() collectgarbage("collect") end)
        pcall(function() collectgarbage() end)
        NotificacaoNightMystic("Vela Hub", "🧹 RAM cleaned!")
    end
});
...
Setting:AddButton({
    ...
    Callback = function()
        ...
        NotificacaoNightMystic("Vela Hub", "✨ NPC effects removed!")
    end
});

Setting:AddButton({
    Name = "🌑 Disable Map Shadows",
    Description = "Chunked — disable CastShadow on the map to reduce GPU usage",
    Callback = function()
        task.spawn(function()
            pcall(function()
                local map = workspace:FindFirstChild("Map")
        ...
        NotificacaoNightMystic("Vela Hub", "🌑 Disabling map shadows...")
    end
});

Setting:AddButton({
    Name = "💡 Disable Post-Processing (Lighting)",
    Description = "Remove Blur, Bloom, SunRays, ColorCorrection, Fog, and Atmosphere",
    Callback = function()
        pcall(function()
        ...
        NotificacaoNightMystic("Vela Hub", "💡 Post-processing removed!")
    end
});
...
Setting:AddButton({
    ...
    Callback = function()
        ...
        NotificacaoNightMystic("Vela Hub", "🎮 Render Quality → Level 1!")
    end
});

Setting:AddToggle({
    Name = "🔄 Auto RAM Clean (every 30s)",
    Description = "Automatically clean junk and dead enemies every 30 seconds",
    Default = false,
    Callback = function(val)
        ...
        if val then
            ...
            NotificacaoNightMystic("Vela Hub", "🔄 Auto RAM Clean: ON")
        else
            NotificacaoNightMystic("Vela Hub", "🔄 Auto RAM Clean: OFF")
        end
    end
});
...
Setting:AddButton({
    ...
    Description = "Vertically stretch the screen to increase FPS",
    Callback = function()
